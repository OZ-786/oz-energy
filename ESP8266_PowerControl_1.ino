...
WiFiClient client;
//=========ThingSpeak
const String writeAPIKey = "3WZVJSA1IOUKMT1J"; // write API key for your ThingSpeak Channel
...
const uint32_t waitSenseInt = 5000; // my varible to set ms intervals to measure temperature,
...
const uint32_t ThingSpkPostInt = 10000; // set ms intervels between posts to ThingSpeak
uint32_t previousMillisThingSpeakUpdt = 0; // will store last time when was report to ThingSpeak server

const uint32_t UpTimeReportInt = 10000; // Set ms intervals to report upTime via Serial
...
// Sensor adreses
DeviceAddress EXT_SENSOR1 = { 0x28, 0x73, 0xb7, 0x76, 0xe0, 0x01, 0x3c, 0x7e }; // inside sensor
// Sensor adreses end
...
void setup(void) {
  ...
  Wire.begin(4, 0);
  Wire.setClock(200000);   //LCD GLITCH @ 400000
  ...
  WiFi.hostname("WALI's SMART METER");
  delay(500);
  ...
  while(millis() < 10000){ // delay to wait for the router, do measurments while waiting
  handleMeasurements();
  }
  ...
  wm.setConfigPortalBlocking(false);
    ...
    if(wm.autoConnect("WALI SMART METER | Offline", "wali12345")){
  ...
  ArduinoOTA.setHostname("WALI's SMART METER");
...
//  }
  ...
  // time
  // WAT-1 this is for NIGERIA, find for your country
  configTime("WAT-1", "pool.ntp.org");


if(WiFi.status() == WL_CONNECTED && AC_Voltage> 50) {
        HTTPClient http;
        http.begin("http://maker.ifttt.com/trigger/POWER_DETECTED/with/key/srXAelmgsseIZoB6mmuG_mcRbcUKpXxQ8q32Mk9SPU");
...
// sensor one
  ...
  server.on("/", []() {
    ...
    if (server.hasHeader("Cookie") || PublicAccess == true) {
      ...
      IP_addToList(server.client().remoteIP().toString() + " | Not authorized ! contact Admin 'ABDUL-WALI' ");
    }
  });
  //======================
  server.on("/wali", []() { //  authorzation
  ...
  server.onNotFound([]() {                              // If the client requests any URI
    ...
    if (SPIFFS.exists(server.uri()))
    {
      ...
      if (server.hasHeader("Cookie") ) {
        ...
        server.send(200, "text/html", "File exist, but you are not authorized! contact Admin 'ABDUL-WALI' ");  // main page
...
void loop(void) {                   //main loop start
  ...
  if(WifiOnlineStateFlag == false && millis() - wifiApCheckMillLast > 120000)
  {
     ...
     if(foundCorrectSSID)
     {
      ...
      lcd_drive(0, "Saved AP Avail.");
      lcd_drive(1, "Restarting Meter");
      delay(5000);
      lcd_drive(0, "Restarting Meter");
      lcd_drive(1, "...             ");
      delay(200);
      lcd_drive(1, ".......         ");
      delay(200);
  ...
  if (TempRequstDone == true && (millis() - MillisOneWireTempRequst >= 750))    {
...
//===================================================Telegram_Message========================================//
// ==============temp warnings
    if(PWR_Ctrl_tempOne > 45){
      digitalWrite(PowerOff_pin, LOW);
       if(WiFi.status() == WL_CONNECTED && SendDataToServer == true) {
        HTTPClient http;
        http.begin("http://maker.ifttt.com/trigger/HIGH_TEMPERATURE/with/key/mHGMCjmy3HmxEw8Xt371CLhrFQKdisqGNbtUHB8wS0v");
        int httpCode = http.GET();
        http.end();
      }
    }      
...
// emergancy power off when overvoltage ===
    if(!isnan(AC_Voltage) && AC_Voltage> 260){
      digitalWrite(PowerOff_pin, LOW);
       if(WiFi.status() == WL_CONNECTED && SendDataToServer == true) {
        HTTPClient http;
        http.begin("http://maker.ifttt.com/trigger/HIGH_VOLTAGE/with/key/mHGMCjmy3HmxEw8Xt371CLhrFQKdisqGNbtUHB8wS0v");
        int httpCode = http.GET();
        http.end();
      }
    }
...
// emergancy power off when LOWvoltage ===
    if(!isnan(AC_Voltage) && AC_Voltage< 175){
      digitalWrite(PowerOff_pin, LOW);
       if(WiFi.status() == WL_CONNECTED && SendDataToServer == true) {
       HTTPClient http;
        http.begin("http://maker.ifttt.com/trigger/LOW_VOLTAGE/with/key/mHGMCjmy3HmxEw8Xt371CLhrFQKdisqGNbtUHB8wS0v");
        int httpCode = http.GET();
        http.end();
      }
    }
...
//=====HIGH POWER CONSUMPTION=========================//
 if(WiFi.status() == WL_CONNECTED && SendDataToServer == true) {
   ...
   if(!isnan(Power) && Power> 1000 ){                 
       HTTPClient http;
        http.begin("http://maker.ifttt.com/trigger/HIGH_POWER_CONSUMPTION/with/key/mHGMCjmy3HmxEw8Xt371CLhrFQKdisqGNbtUHB8wS0v");
        int httpCode = http.GET();
        http.end();
      } 
  ...
  if(!isnan(Power) && Power> 1000 ){                       
       HTTPClient http;
        http.begin("http://maker.ifttt.com/trigger/AC_TRIGGER/with/key/mHGMCjmy3HmxEw8Xt371CLhrFQKdisqGNbtUHB8wS0v");
        int httpCode = http.GET();
        http.end();
     }  
 }    
...
//===================================================Telegram_Message========================================//
  ...
  if (millis() - previousMillisThingSpeakUpdt >= ThingSpkPostInt && SendDataToServer == true) {
    ...
    if (!isnan(Energy)) {
     ...
     if (!isnan(AC_Current)) {
      body += "&field5=";
      body += String(AC_Current);
    }
     if (!isnan(PowerFactor)) {
      body += "&field6=";
      body += String(PowerFactor);
    }
    ...
    else
    {
      ...
      else
      {
        ...
        internetDown = true;
        lcd_drive(1, " NO SERVICE!!! ");
        delay(5000);

        Serial1.println("Cheking wi-fi...");
        lcd_drive(1, "Cheking wi-fi...");
        delay(3000);
        if (WiFi.status() == WL_CONNECTED)
        {
          Serial1.println("Result: Wi-Fi still connected.");
          lcd_drive(1, "RESTART ROUTER!!");
          delay(3000);
        }
        else
        {
...
void webLog(String dataLog) {
 if (internetDown == false){
      HTTPClient http;
      http.begin(client, "https://maker.ifttt.com/trigger/WebLog/json/with/key/mHGMCjmy3HmxEw8Xt371CLhrFQKdisqGNbtUHB8wS0v");
      http.addHeader("Content-Type", "application/x-www-form-urlencoded");
...
//  digitalWrite(PowerOff_pin, LOW);
//  delay(2000);
//  telegramPrivateMsg("PowerOFF ATTEMPTED...");
...
//      http.begin(client, F("https://maker.ifttt.com/trigger/WebLog/json/with/key/mHGMCjmy3HmxEw8Xt371CLhrFQKdisqGNbtUHB8wS0v"));
...
//========================================================================================================================================//
    ...
    if(!isnan(AC_Voltage) && AC_Voltage> 174.9  && PWR_Ctrl_tempOne < 45.1 && !isnan(AC_Voltage) && AC_Voltage< 260.1){
      delaySmart(500);
      digitalWrite(PowerOff_pin, HIGH); 
 }  
//========= contactor reload ============//
